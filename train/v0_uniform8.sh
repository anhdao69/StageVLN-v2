...
echo ">>>>> attention=$ATTN_IMPLEMENTATION deepspeed=$(basename "$DEEPSPEED_CONFIG") optimizer=$OPTIM sparse_action_logits=$SPARSE_ACTION_LOGITS"
echo ">>>>> micro_batch=$PER_DEVICE_TRAIN_BATCH_SIZE gradient_accumulation=$GRADIENT_ACCUMULATION_STEPS gradient_checkpointing=$GRADIENT_CHECKPOINTING"
